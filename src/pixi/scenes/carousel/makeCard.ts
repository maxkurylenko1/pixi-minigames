...
export function makeCard(index: number, icon?: Texture, color: number = 0x4cc9f0) {
  ...
  // ◻️ маленький квадрат по центру левого блока (подложка под иконку)
  const markerSize = 20;
  const marker = new Graphics()
    .rect(-markerSize / 2, -markerSize / 2, markerSize, markerSize)
    .fill(shade(color, 0.35))
    .stroke({ width: 1, color: shade(color, -0.25) });
  marker.position.set(32, ITEM_H / 2); // центр левого блока (ширина 64)
  c.addChild(marker);
  ...
  // иконка (если есть) — поверх маркера
  if (icon) {
  ...
  // 🖼️ рамка вокруг айтема (чёткая 1px)
  // 0.5-смещение даёт crisp-линию на пиксельной сетке
